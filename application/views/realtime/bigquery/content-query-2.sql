...
FROM (
  ...
  FROM (
    SELECT
      ...
      cat3,
      content_id,
      RANK() OVER (PARTITION BY team_id, cat1, cat2, cat3 ORDER BY score DESC) toplist_rank,
      score
    FROM (
      SELECT
        team_id,
        ...
        m * (
            IF(
                ...
                IF(x1 / x2 > 1, IF(x1 / x2 < 10, x1 / x2, 10), 1),
                1
            )
        ) score
      FROM (
          SELECT
            ...
            content_id,
            SUM(is_share) m,
            SUM(IF(window == SEC_TO_TIMESTAMP(FLOOR({{from}} / (150 * 60)) * 150 * 60), is_share, 0)) x1,
            SUM(IF(window == SEC_TO_TIMESTAMP(FLOOR(({{from}} - (150 * 60)) / (150 * 60)) * 150 * 60), is_share, 0)) x2,
            SUM(IF(window == SEC_TO_TIMESTAMP(FLOOR(({{from}} - (2 * 150 * 60)) / (150 * 60)) * 150 * 60), is_share, 0)) x3
          FROM (
            SELECT
              team_id,
              {{cat1}} AS cat1,
              ...
              content_id,
              is_share,
              SEC_TO_TIMESTAMP(FLOOR(timestamp_to_sec(time) / (150 * 60)) * 150 * 60) window
